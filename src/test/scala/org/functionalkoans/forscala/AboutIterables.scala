...
class AboutIterables extends KoanSuite with ShouldMatchers {
  ...
  koan( """`grouped` will return a fixed sized Iterable chunks of an Iterable""") {
  ...
  koan( """`sliding` can take the size of the window as well as the size of the step during each
          | iteration""") {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
